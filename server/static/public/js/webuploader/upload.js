...
// 当domReady的时候开始初始化
function imagesUploader(id, data) {
  var $wrap = $(id),
    // 图片容器
    $queue = $('<ul class="filelist" id="filelist"></ul>').appendTo(
      $wrap.find(".queueList")
    ),
    // 状态栏，包括进度和控制按钮
  ...
  function addEidtFile(file){
    var $li = $(
        '<li id="' +
          file.id +'"'+'class="state-complete" '+
          'pathname="'+file.pathName+ '" filename="'+file.fileName+
          '" >' +
          '<p class="title">' +
          file.fileName +
          "</p>" +
          '<p class="imgWrap"><img src="'+file.pathName+'"/>'+'</p>' +
          '<p class="progress"><span></span></p>' +
          "</li>"
      ),
      $btns = $(
