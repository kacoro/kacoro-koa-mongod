...
class Index extends Component {
  ...
  async componentDidMount() {
    // let checkInit = JSON.stringify(this.props.initialData) === "{}"
      const res = await http.get.bind(this)({url:`admin/comment${this.props.location.search}`})
      this.setState({
        data: res.data,
  ...
  handleDelete =async(index)=>{
    ...
    list.splice(index,1)
    const res = await http.delete.bind(this)({url:`admin/comment/${id}`})
    if(res){
  ...
  shouldComponentUpdate = async(nextProps) => {
    if (nextProps.location !== this.props.location) {
      const res = await http.get.bind(this)({url:`admin/comment${nextProps.location.search}`})
      if(res){
        this.setState({
          data: res.data,
          pagination:res.pagination
  ...
  async getCurrentPage(currentPage) {
    ...
    this.props.history.push(`/admin/comment?${qs.stringify(obj)}`)
  }
  creatList(){
    ...
    const listItems = data.map((item, index) =>
        <tr key={index+(pagination.page-1)*10+1}>
          ...
          <td><Link to={`/admin/comment/edit/${item._id}`}>{item.name || item.nickname}</Link></td>
          <td> 
            {item.content}</td>
